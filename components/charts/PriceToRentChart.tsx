...
interface PriceToRentChartProps {
  ...
  // We need countryCode now to exclude it from comparisons
  countryCode?: string; 
  isComparing?: boolean;
}
...
export function PriceToRentChart({ countryData, countryCode = "", isComparing = false }: PriceToRentChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    base.forEach(d => dataMap.set(d.year, { year: d.year, [mainKey]: d.val }));
    ...
    const codeMetrics = allCodes.map(c => {
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Line
            type="monotone"
            dataKey={mainKey}
            name={countryDisplayNames[mainKey as keyof typeof countryDisplayNames] ?? t("PriceToRent.legend")}
            stroke="var(--color-accent)"
            strokeWidth={3}
            dot={{ r: 3 }}
          />
          {isComparing && comparisonCodes.map((code: string, index: number) => (
             <Line
                key={code}
                type="monotone"
                dataKey={code}
